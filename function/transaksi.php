...
function tambahTransaksi($post)
{
    ...
    if(!$ongkir) {
        $_SESSION['pesan'] = 'Pilih kota terlebih dahulu';
        return header('location:' . url . 'user/cekOut.php');
    }


    mysqli_query($konek, "INSERT INTO transaksi VALUES(
        '', '$id_pesan','$id_user',  null, '$penerima', '$alamat', '$telepon', '$email', '$kuantiti_total', '$total_akhir', '$pembayaran', '$id_status', '$pesan', '', '', '$ongkir' 
        )");
    ...
    return header('location:../user/profil.php?tab=transaksi');
...
function bayar($post)
{
    ...
    if ($img['error'] == 4) {
        ...
        return header('location:' . url . 'user/profil.php?tab=transaksi');
