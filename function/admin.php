...
function home()
{
    ...
    $thisMonth = date('m');
    $thisYear = date('Y');
    $totalAkhir = mysqli_query($konek, "SELECT SUM(total_akhir) as total_akhir FROM transaksi");
    $totalAkhirThisMonth = mysqli_query($konek, "SELECT SUM(total_akhir) as total_akhir FROM transaksi WHERE YEAR(pesan_at) = {$thisYear} AND MONTH(pesan_at) = {$thisMonth}");
    ...
    $data = [
        ...
        'trans'     => $tran,
        'totalAkhir'     => mysqli_fetch_object($totalAkhir),
        'totalAkhirThisMonth'     => mysqli_fetch_object($totalAkhirThisMonth),
    ];
    return $data;
}
...
function ubahProduk($id, $post)
{
    ...
    if ($sql > 0) {
        return header('location:' . url . 'admin/detailProduk.php?id='.$id  .'&pesan=Data berhasil Diubah');
    }
...
function ambilTransaksi()
{
    ...
    $result = mysqli_query($konek, "SELECT * FROM transaksi JOIN status ON status.id_status = transaksi.id_status WHERE transaksi.id_status != '3' ORDER BY transaksi.id DESC");
    $trans = [];
...
function verifiTransaksi($post)
{
    global $konek;

    $pengirim = $post['pengirim'];
    $id_pesan = $post['idpesan'];
    mysqli_query($konek, "UPDATE transaksi SET id_status='1', pengirim='$pengirim' WHERE id_pesan = '$id_pesan' ");
...
function cekBayar($id)
{
    ...
    $pembayaran = mysqli_query($konek, "SELECT * FROM pembayaran WHERE id_pesan='$id'");
    $transaksi = mysqli_query($konek, "SELECT * FROM transaksi WHERE id_pesan='$id'");

    return array(
        mysqli_fetch_object($pembayaran),
        mysqli_fetch_object($transaksi)
    );
}
